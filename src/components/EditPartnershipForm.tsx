...
const EditPartnershipForm: React.FC<Props> = ({ partnership, onClose }) => {
  ...
  useEffect(() => {
    if (partnership) {
      setForm({
        ...
        programType: partnership.programType || 'Adoption',
        geographicFocus: partnership.geographicFocus || '',
        beneficiaries: partnership.beneficiaries || '',
        fundingSources: partnership.fundingSources || '',
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const resp = await fetch(`/api/partnerships?id=${partnership.id}`, {
        method: 'PUT',
      ...
      if (resp.ok) {
        ...
        toast.error(err?.error || 'Failed to update partnership');
      }
    } catch (e) {
      console.error(e);
